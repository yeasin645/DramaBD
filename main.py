...
# ==========================================
# ২. টেলিগ্রাম ১৯টি কমান্ড (বিন্দু পরিমান মিসিং নেই)
# ==========================================
...
# ==========================================
# ৩. ওয়েব ডিজাইন (৪ কলাম ইপিসোড + মনিটেগ ফিক্স)
# ==========================================
...
<head>
    ...
    <style>
        ...
        .logo span { background: #ff0000; padding: 2px 6px; border-radius: 4px; margin-left: 4px; }
...
<head>
    ...
    <style>
        ...
        .top-nav { background: #111; padding: 12px; display: flex; justify-content: space-between; border-bottom: 2px solid #ff0000; margin-bottom: 20px; }
        .poster { width: 85%; max-width: 300px; border-radius: 15px; border: 3px solid #ff0000; box-shadow: 0 0 20px rgba(255,0,0,0.5); }
        .timer-msg { background: #ff0000; padding: 12px; margin: 15px; border-radius: 10px; font-weight: bold; display: none; }
        
        /* ৪ কলাম গ্রিড */
        .ep-grid { display: grid; grid-template-columns: repeat(4, 1fr); gap: 8px; padding: 10px; }
        .btn-ep { 
            padding: 12px 2px; border-radius: 8px; font-weight: bold; font-size: 11px; 
        ...
        .c1 { background: #007bff; } .c2 { background: #e91e63; } .c3 { background: #4caf50; } .c4 { background: #673ab7; }
        .get-btn { background: #ffc107 !important; color: #000 !important; display: none; }
        .btn-movie { background: #ff0000; color: #fff; padding: 15px; border-radius: 12px; width: 90%; margin: 15px auto; font-weight: bold; display: block; text-decoration: none; }
    </style>
</head>
<body>
    <div class="top-nav">
        <button onclick="location.href='/'" class="btn btn-sm btn-outline-light">🏠 Home</button>
        <span style="color:red; font-weight:bold;">Premium Player</span>
        <button onclick="location.reload()" class="btn btn-sm btn-danger">🔄 Reload</button>
    </div>
    ...
    <div id="unlock-section">
        {% if item.type == 'movie' %}
            {% for l in item.links %}
            <div id="box-{{ l.uid }}">
                <button id="btn-{{ l.uid }}" class="btn-movie" onclick="startAd('{{ l.uid }}')">🔓 UNLOCK {{ l.q }} FILE</button>
                <div id="get-{{ l.uid }}" style="display:none;"><a href="https://t.me/{{ bot_u }}?start={{ l.uid }}" class="btn-movie" style="background:#28a745;">📥 DOWNLOAD NOW</a></div>
            </div>
            {% endfor %}
        {% else %}
            <div class="ep-grid">
            {% for e in item.episodes %}
                <div id="box-{{ e.uid }}">
                    ...
                    <a id="get-{{ e.uid }}" href="https://t.me/{{ bot_u }}?start={{ e.uid }}" class="btn-ep get-btn" style="background:#ffc107; color:#000;">GET</a>
                </div>
            {% endfor %}
            </div>
        {% endif %}
    ...
    <script>
        ...
        function updateTimer() {
            ...
            if(activeKey) {
                ...
                if(remaining > 0) {
                    ...
                    msgBox.innerHTML = `🔐 লক হবে: ${Math.floor(remaining/60)}মি. ${remaining%60}সে. পর`;
        ...
        function startAd(uid) {
            ...
            if (adStartedAt === 0) {
                if (typeof window[sdkFn] === 'function') {
                    window[sdkFn](); // Calling Monetag Show Function
                    adStartedAt = new Date().getTime();
                    alert("এড লোড হয়েছে। দয়া করে " + adTimer + " সেকেন্ড দেখুন।");
                } else {
                    alert("এড লোড হতে সমস্যা হচ্ছে। পেজ রিফ্রেশ দিন।");
                }
                return;
            }
        ...
        window.onload = () => { checkPersistence(); setInterval(updateTimer, 1000); };
    </script>
</body>
</html>
"""
...
# ==========================================
# ৪. সার্ভার রুটস এবং মেইন রান (অক্ষত)
# ==========================================
